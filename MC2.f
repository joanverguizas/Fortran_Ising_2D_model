C Joan Verguizas I Moliner
C Project: 2D Ising model
C In order to see the theoretical model that we are working with go to the pdf file.


      program pr3
      implicit none
C VARIABLE DECLARATION      
      integer*4 SEED, I,J,L,N,k,MCINI,MCD,NSEED,SEED0,NPASSOS
      PARAMETER (L=60) !!! Be aware that for L=60 the total computational time for executing this program is around 6 days. I have calculated this program for
C L=60,30 and 15. As you increase L the result will get closer to an ideal 2D Ising model but also the computational time will increase heavily.  
      integer*4 PBC(0:L+1),MCTOT,SUMA,IPAS,IMC,DE,ITEMP
      integer*2 S(1:L,1:L)
      real*8 genrand_real2,MAG,MAGNE,energ,ENE,TEMP,DELTA,ENEBIS,
     + genrand_real3,W(-8:8),VARM,VARE,SUMM2,SUMM,SUME2,SUME,SUMAM,
     + SUM,e,epsie,m,mabs,m2arrel,epsim,CV,CVn,X,Xn,TINI,TFIN,
     + INTERVAL
      real*4 TIME1,TIME2
      character*29 NOM
      character*30 DATE
...
C INITIALIZATION OF VARIABLES
      ...
      TFIN=3.4d0 
      INTERVAL=0.01d0
      NPASSOS=INT((TFIN-TINI)/(INTERVAL)) ! Number of steps for the temperature
      NSEED=200
      SEED0=117654
      MCTOT=40000   !MonteCarlo steps taken
      MCINI=2000
      MCD=20
      NOM='SIM-L-060-TEMP-200-MCTOT-40K-2'
C CALCULATED VARIABLES
      N=L*L
C

C DEFINING OUR PBC/LATTICE
      ...
      do ITEMP=0,NPASSOS
         Call CPU_TIME(TIME1)
         TEMP = TINI + ITEMP*INTERVAL
C We initialize thess variables in order to be able to calculate some quantities later. 
C SUM: Number of independent configurations, SUME: energy value, SUME2: squared energy value, 
C SUMM: magnetization value, SUMM2: squared magnetization value, SUMAM: absolute magnetization value      
         SUM=0.d0
         SUME=0.d0
         SUME2=0.d0
         SUMM=0.d0
...
C 
         do DE=-8,8
            W(DE)=dexp(-dfloat(DE)/TEMP)    ! We tabulate the exponential (Boltzman condition) in order to lower the exponential time
         enddo
      
         do SEED=SEED0,SEED0+NSEED-1,1      
            call init_genrand(SEED)
            do i=1,L
               do j=1,L                                   ! We call MT19937 subroutine and functions to generate random numbers
            ...
            do IMC=1,MCTOT                 ! Metropolis algorism
               do IPAS=1,N
                  I=int(genrand_real2()*L+1)
                  J=int(genrand_real2()*L+1)
                  suma=S(I,PBC(J+1)) + S(I,PBC(J-1)) + S(PBC(I+1),J) +
     +            S(PBC(I-1),J)
                  DE=2*S(I,J)*suma
                  if (DE.lt.0) then      ! If the energy is lowered we accept the change
                     S(I,J)=-S(I,J)
                     ENE=ENE+DE
                  else
                     DELTA=genrand_real3()               ! Here the change hasn't been accepted so we call a random number
                     if (DELTA.lt.W(DE)) then
                        S(I,J)=-S(I,J)                   ! If this number is lower to the Boltzmann factor at the current T
                        ENE=ENE+DE                       ! the change is also accepted otherwise is not.
                     else
                        S(I,J)=S(I,J)
                     endif
                  endif
...
c CALCULATION THE VARIABLES WE WANT TO REPRESENT BEING THOSE e:energy by spin, epsie: error of e, 
C m: magnetization by spin, mabs: absolute value of the magnetitzation, m2arrel: square root of m²,
C epsim: error of m, CV: heat capacity of our system, cv: heat capacity by spin, 
C X: magnetic susceptibility, xn: magnetic susceptibility per spin.            

         e=SUME/DBLE(N)
         epsie=(dsqrt((SUME2-SUME**2)/SUM))/dble(N)
         m=SUMM/dble(N)
         mabs=SUMAM/dble(N)
         m2arrel=dsqrt(SUMM2/dble(N**2))
         epsim=(dsqrt((SUMM2-SUMM)/SUM))/dble(N)
         CV=(SUME2-SUME**2)/(TEMP**2)
         CVn=CV/dble(N)
         X=(SUMM2-SUMM**2)/TEMP
         Xn=X/dble(N)
c

         write(13,*) L,TEMP,e,epsie,m,mabs,m2arrel,epsim,CV,CVn,X,Xn
         Call CPU_TIME(TIME2)
